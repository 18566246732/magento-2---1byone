...
class ReviewManagement implements ReviewInterface
{
    ...
    public function __construct(
        ...
        ReviewAdvancedExtensionFactory $reviewAdvancedExtensionFactory,
    ...
    )
    {
        ...
        $this->reviewAdvancedExtensionFactory = $reviewAdvancedExtensionFactory;
    ...
    public function getReviewsList($productId)
    {
        ...
        foreach ($collection as $reviewCollection) {
            ...
            if ($reviewCollection->getExtensionAttributes() != null) {
                $extensionAttribute = $reviewCollection->getExtensionAttributes();
                $extensionData = $extensionAttribute->getReviewAdvanced();
            }
            $reviewArray[] = $data;
        }
        $reviewData[] = [
     ...
     * @param int $storeId
     *
     * @param ReviewAdvanced $reviewAdvanced
     * @return array
            ...
            if ($validate === true) {
                try {
                    $review->setEntityId($review
                        ...
                        ->setStoreId($storeId)
                        ->setStores([$storeId]);
                    $this->addReviewAdvanced($review, $reviewAdvanced);
                    $review->save();
                    if (count($ratings)) {
    ...
    private function addReviewAdvanced(Review $review, ReviewAdvanced $advancedData)
    {
        $extensionAttributes = $review->getExtensionAttributes();
        if (empty($extensionAttributes)) {
            $extensionAttributes = $this->reviewAdvancedExtensionFactory->create();
        }
        $extensionAttributes->setReviewAdvanced($advancedData);
